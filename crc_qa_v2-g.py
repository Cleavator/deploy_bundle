...
def generate_subqueries(query, api_key):
    ...
    print(f"[DEBUG] generate_subqueries response: {response[:200]}..." if response else "[DEBUG] response empty")
    ...
    try:
        ...
        if not isinstance(data, dict):
            print(f"[WARNING] generate_subqueries expected dict, got {type(data)}. Fallback to original query.")
            return {"subqueries": [query], "required_points": []}
            
        return data
    except Exception as e:
        print(f"[ERROR] generate_subqueries parsing failed: {e}")
        return {"subqueries": [query], "required_points": []}
